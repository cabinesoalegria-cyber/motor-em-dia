 ...
 * JSX portado diretamente do repositório Lovable (motor-em-dia-hub).
 * Adaptações mínimas: createFileRoute removido, PrimaryButton/SecondaryButton
 * convertidos para next/link, hrefs atualizados para rotas reais.
 * Design tokens definidos em lovable.css (escopo .med-ld).
 */
...
/* ─── Sub-components (idênticos ao Lovable) ─────────────────────────────── */
...
/**
 * PrimaryButton do Lovable → PrimaryLink com next/link
 * href padrão: /cadastro (rota de criação de conta)
 */
...
/**
 * SecondaryButton do Lovable → SecondaryLink com next/link
 */
...
/* ─── Componente principal da rota /landing ─────────────────────────────── */

export default function LandingPageContent({ source = 'landing' }: { source?: string }) {
  return (
    <div className="med-ld min-h-screen bg-background text-foreground">
      <Header />
      <Hero />
      <FeatureBar />
      <ProblemSection />
    ...
    <footer className="border-t border-border bg-background py-8">
      <Container className="flex flex-col items-center justify-between gap-5 sm:flex-row">
        ...
        <div className="flex items-center gap-6 text-sm font-medium text-ink">
          ...
          <Link href="/cadastro" className="hover:text-primary">Criar conta grátis</Link>
        </div>
      </Container>
    </footer>
  );
